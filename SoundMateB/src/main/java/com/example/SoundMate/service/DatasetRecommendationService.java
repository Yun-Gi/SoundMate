    ...
    public List<Map<String, Object>> recommendSongs(Map<String, Object> inputFeatures) {
        ...
        List<Map<String, Object>> fullRecommendations = musicDataset.stream()
                .map(track -> {
                    ...
                    String trackName = track.getOrDefault("track_name", "").trim();
                    String artistName = track.getOrDefault("artist_name", "").trim();
    
                    Map<String, Object> song = new HashMap<>();
                    song.put("track_name", trackName);
                    song.put("artist_name", artistName);
                    song.put("distance", distance);
                    return song;
                })
                .sorted(Comparator.comparingDouble(song -> (double) song.get("distance")))
                .collect(Collectors.toList());
    
        // 상위 5개만 추림
        List<Map<String, Object>> topRecommendations = fullRecommendations.stream()
                .limit(5)
                .collect(Collectors.toList());
    
        // 로그 출력
        System.out.println("추천 결과:");
        for (Map<String, Object> song : topRecommendations) {
